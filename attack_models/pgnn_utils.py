...
def precompute_dist_data(edge_index, num_nodes, approximate=0):
    ...
    for node_i in graph.nodes():
        ...
        for node_j in shortest_dist.keys():
            dists_array[node_i, node_j] = 1 / (shortest_dist[node_j] + 1)
    return dists_array
...
def preselect_anchor(num_nodes, dists, c=1, anchorset_n=None, layer_num=1, anchor_num=32, anchor_size_num=4, device='cpu'):
    if (anchorset_n == None):
        anchorset_n = num_nodes
    # anchor_set = []
    ...
    anchorset_id = get_random_anchorset(anchorset_n,c=c)
    # data.dists_max, data.dists_argmax = get_dist_max(anchorset_id, dists, device)
    return get_dist_max(anchorset_id, dists, device)


def preprocess (edge_index, num_nodes, use_saved=False, data_dir='data/Planetoid'):
    ...
    # if args.task=='link':
    #     # train_mask would actually be links only so good gracious
    #     dists_removed = precompute_dist_data(data.train_mask, data.num_nodes, approximate=1)
    #     dists_removed_list.append(dists_removed)
    #     data.dists = torch.from_numpy(dists_removed).float()
    #     data.edge_index = torch.from_numpy(duplicate_edges(data.train_mask)).long()
    # else:
    # can have just this no ? I guess it's fine (the above is just to reduce size I guess)
    # we can replace data.edge_index by some candidate set as well...
    file_dir = os.path.dirname(__file__)
    if (use_saved):
        if (os.path.exists(f"{file_dir}/../{data_dir}/dists.pt")):
            return torch.load(f"{file_dir}/../{data_dir}/dists.pt")
        else:
            dists = precompute_dist_data(edge_index.cpu().numpy(), num_nodes, approximate=1)
            dists = torch.from_numpy(dists).float()
            torch.save(dists, f"{file_dir}/../{data_dir}/dists.pt")
    else:
        dists = precompute_dist_data(edge_index.cpu().numpy(), num_nodes, approximate=1)
        dists = torch.from_numpy(dists).float()
    # torch.save(dists, "temp_dists.pt")
    return dists
