...
try:
    from gcn_conv_guard import GCNConv # GINConv, GATConv, JumpingKnowledge
except:
    from attack_models.gcn_conv_guard import GCNConv # GINConv, GATConv, JumpingKnowledge
...
class GCN_Guard(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, drop=False, weight_decay=5e-4, n_edge=1,with_relu=True,
                 with_bias=True, device=None, down_task="node_classification", attention=True):

        super(GCN_Guard, self).__init__()

        assert device is not None, "Please specify 'device'!"
        self.down_task = down_task
        if (down_task in ["link_prediction", "link_pair"]):
        ...
        if (self.down_task == "node_classification"):
            ...
            self.gc2 = GCNConv(nhid, nhid, bias=True,)
    ...
    def forward(self, x, adj, sparse_input=True):
        ...
        """GCN and GAT"""
        if self.attention:
            adj = self.att_coef(x, adj, i=0, sparse_input=sparse_input)
        x = self.gc1(x, edge_index, edge_weight=edge_weights)
        x = F.relu(x)
        # x = self.bn1(x)
        if self.attention:  # if attention=True, use attention mechanism
            ...
            adj_values = edge_weights

        x = F.dropout(x, self.dropout, training=self.training)
        x = self.gc2(x, edge_index.to(self.device), edge_weight=adj_values.to(self.device))
    ...
    def att_coef(self, fea, edge_index, is_lil=False, i=0, sparse_input=True):
        if (sparse_input):
            if is_lil == False:
    ...
    def fit(self, features, adj, train_labels, val_labels, idx_train, idx_val=None, idx_test=None, train_iters=81, att_0=None,
        ...
        if idx_val is None:
            self._train_without_val(train_labels, idx_train, train_iters, verbose)
        else:
            if patience < train_iters:
                self._train_with_early_stopping(train_labels, val_labels, idx_train, idx_val, train_iters, patience, verbose)
            else:
                self._train_with_val(train_labels, val_labels, idx_train, idx_val, train_iters, verbose)

    def _train_without_val(self, train_labels, idx_train, train_iters, verbose):
        self.train()
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        for i in range(train_iters):
            optimizer.zero_grad()
            output = self.forward(self.features, self.adj_norm)
            output = self.predict(z=output[idx_train])
            # loss_train = F.nll_loss(output, train_labels, weight=None)   # this weight is the weight of each training nodes
            loss_train = self.loss(output, train_labels, weight=None)   # this weight is the weight of each training nodes
            loss_train.backward()
            optimizer.step()
        ...
        self.eval()
        # output = self.forward(self.features, self.adj_norm)
        # output = self.predict(z=output)

    def _train_with_val(self, train_labels, val_labels, idx_train, idx_val, train_iters, verbose):
        if verbose:
        ...
        for i in range(train_iters):
            ...
            loss_train = self.loss(output, train_labels)
            loss_train.backward()
            optimizer.step()
            self.eval()

            output = self.forward(self.features, self.adj_norm)
            output = self.predict(z=output[idx_val, :])

            # loss_val = F.nll_loss(output, val_labels)
            loss_val = self.loss(output, val_labels)
            output = self.forward(self.features, self.adj_norm)
            pred = self.predict(z=output[idx_val, :], classify=True)
            val_correct = pred == val_labels  # Check against ground-truth labels.
            acc_val = int(val_correct.sum()) / int(len(val_correct))
            # acc_val = utils.accuracy(output, val_labels)
    ...
    def _train_with_early_stopping(self, train_labels, val_labels, idx_train, idx_val, train_iters, patience, verbose):
        if verbose:
            print('=== training gcn model ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
    ...
    def test(self, idx_test, test_labels):
        self.eval()
        z = self.forward(self.features, self.adj_norm)
        output = self.predict(z[idx_test, :])  # here use the self.features and self.adj_norm in training stage
        # loss_test = F.nll_loss(output, test_labels)
        try:
            loss_test = self.loss(output, test_labels)
        except:
            loss_test = self.loss(output, test_labels.float())
        pred = self.predict(z=z[idx_test, :], classify=True)
        test_correct = pred == test_labels  # Check against ground-truth labels.
        acc_test = int(test_correct.sum()) / int(len(test_correct))
        # acc_test = utils.accuracy(output, test_labels)
    ...
    def predict(self, z=None, classify=False, to_flatten=True):
