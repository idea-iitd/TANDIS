...
class NstepReplaySubMemCell(object):
    ...
    def add(self, s_t, a_t, r_t, s_prime):
    ...
    def add_list(self, list_st, list_at, list_rt, list_sp):
        for i in range(len(list_st)):
            if list_sp is None:
                sp = (None, None, None)
            else:
                sp = list_sp[i]
            self.add(list_st[i], list_at[i], list_rt[i], sp)
...
class NstepReplayMemCell(object):
    ...
    def add(self, s_t, a_t, r_t, s_prime):
        if not self.balance_sample or r_t < 0:
            self.sub_list[0].add(s_t, a_t, r_t, s_prime)
        else:
            assert r_t > 0
            ...
            self.sub_list[1].add(s_t, a_t, r_t, s_prime)
    ...
    def sample(self, batch_size):
        ...
        list_st, list_at, list_rt, list_s_primes = self.sub_list[0].sample(batch_size // 2)
        list_st2, list_at2, list_rt2, list_s_primes2 = self.sub_list[1].sample(batch_size - batch_size // 2)
        
        return list_st + list_st2, list_at + list_at2, list_rt + list_rt2, list_s_primes + list_s_primes2
